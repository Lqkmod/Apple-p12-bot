...
def run(sdt, i):
    with concurrent.futures.ThreadPoolExecutor(max_workers=30) as executor:
        futures = [executor.submit(fn, sdt) for fn in otp_functions]  # Sử dụng otp_functions
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python sms.py <sdt> <count>")
        sys.exit(1)
    
    sdt = sys.argv[1]  # Lấy số điện thoại từ tham số dòng lệnh
    count = int(sys.argv[2])  # Lấy số lượng từ tham số dòng lệnh
