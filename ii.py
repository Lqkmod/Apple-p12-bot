...
def is_key_valid():
    try:
        with open(tiktok_FILE, "r") as f:
            tiktok = json.load(f)
            saved_key = tiktok.get("key")
            expiration_time_str = tiktok.get("expiration_time")
...
class Zefoy:
    ...
    def get_captcha(self):
        ...
        try:
            ...
            print('Giải mã xác thực..')
            return False
        except Exception as e:
            print(f"\033[1;33mKhông giải được captcha: {e}")
            time.sleep(2)
            self.get_captcha()
    ...
    def send_captcha(self, new_session = False):
        ...
        if 'Enter Video URL' in request.text: 
            print('Phiên đã được tạo')
            open('session','w',encoding='utf-8').write(self.session.cookies.get('PHPSESSID'))
            print(f"Đã giải mã xác thực thành công: {captcha_solve}")
            self.video_key = request.text.split('" placeholder="Enter Video URL"')[0].split('name="')[-1]
    ...
    def get_table(self, i = 1):
        ...
        for service in self.services:
            ...
            i += 1
        table.title = f"{Fore.WHITE} VIEW TIKTOK KHANGLEE: {len([x for x in self.services_status if self.services_status[x]])}{Fore.RESET}"
        print(table)
    ...
    def find_video(self):
        ...
        while True:
            ...
            if 'Session expired. Please re-login' in self.video_info:
                print('Phiên đã hết hạn. Đang đăng nhập lại...')
                self.send_captcha()
                return
                ...
                while time.time()-2<_+zefoy:
                    t-=1
                    print('\033[31m[KHANHLEE]=>Please what: \033[1;32m{0}\033[31m '.format(t)+'second\033[1;32m',end='\r')
                    time.sleep(1)
                continue
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                print(f'\033[1;31mLỖI NGHIÊM TRỌNG | THỬ SAU |d {e}')
                time.sleep(10)
